  ...
  >
    ...
    <a-form :form="form" layout="vertical" class="main-form">
      <a-row :gutter="24" class="main-content-row">
        <a-col :span="10" class="map-column">
          <div id="areas" class="map-container"></div>
        </a-col>
        <a-col :span="5" class="map-column">
          <h4>推荐路线</h4>
          <div></div>
...
export default {
  ...
  methods: {
    ...
    calculateRoute (startLng, startLat, endLng, endLat) {
      ...
      const totalRequests = policies.length

      policies.forEach((policyInfo, index) => {
        const driving = new BMapGL.DrivingRoute(this.map, {
          renderOptions: {
            map: null, // 不自动渲染到地图上，由selectRoute控制
            autoViewport: false
          },
          ...
          onSearchComplete: (results) => {
            ...
            if (driving.getStatus() === BMAP_STATUS_SUCCESS) {
              const planCount = results.getNumPlans()

              for (let i = 0; i < planCount; i++) {
                const plan = results.getPlan(i)
                const routeInfo = {
                  index: this.routePlans.length, // 使用当前数组长度作为索引
                  distance: plan.getDistance(true), // 距离
                  duration: plan.getDuration(true), // 时间
                  policyName: policyInfo.name,
                  policyCode: policyInfo.code,
                  routes: plan
                }
